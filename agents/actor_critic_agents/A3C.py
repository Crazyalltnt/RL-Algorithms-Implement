...
class A3C(Base_Agent):
    ...
    def locally_load_policy(self):
        ...
        self.actor_critic_optimizer.load_state_dict(model['actor_critic_optimizer'])
    ...
    #         with torch.no_grad():
    #             action = self.actor_critic(state)[:, list(range(self.action_size))].cpu().data.numpy()
    #         action = action[0]
    #         self.action = action
...
class Actor_Critic_Worker(torch.multiprocessing.Process):
    """玩指定 episodes 游戏的 Actor critic worker"""
    ...
    def calculate_new_exploration(self):
        """计算新的探索参数 epsilon。 它在当前 epsilon 上下 3 倍范围内随机选择一个点"""
        with self.counter.get_lock():
    ...
    def calculate_discounted_returns(self):
        """计算一个 episode 的累积折扣回报，然后我们将在学习迭代中使用它"""
    ...
    def normalise_discounted_returns(self, discounted_returns):
        """通过除以该 episode 的平均回报和标准差来标准化折扣回报"""
