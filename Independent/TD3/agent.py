...
class TD3(object):
    def __init__(self, state_dim, action_dim, max_action, cfg):
        ...
        self.policy_freq = cfg.policy_freq  # 值critic更新频率大于策略actor
        ...
        if self.total_it % self.policy_freq == 0:
            ...
            for param, target_param in zip(self.critic.parameters(), self.critic_target.parameters()):
                target_param.data.copy_(
                    self.lr * param.data + (1 - self.lr) * target_param.data)
    ...
    def save(self, path):
        ...
        torch.save(self.critic_optimizer.state_dict(), path + "td3_critic_optimizer.pt")

        torch.save(self.actor.state_dict(), path + "td3_actor.pt")
        torch.save(self.actor_optimizer.state_dict(), path + "td3_actor_optimizer.pt")

    def load(self, path):
        self.critic.load_state_dict(torch.load(path + "td3_critic.pt"))
        self.critic_optimizer.load_state_dict(torch.load(path + "td3_critic_optimizer.pt"))
        self.critic_target = copy.deepcopy(self.critic)

        self.actor.load_state_dict(torch.load(path + "td3_actor.pt"))
        self.actor_optimizer.load_state_dict(torch.load(path + "td3_actor_optimizer.pt"))
        self.actor_target = copy.deepcopy(self.actor)
