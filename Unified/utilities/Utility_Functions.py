...
def create_cur_run_data_dir():
    """创建运行时数据保存目录"""
    time_now = time.strftime("%Y%m%d-%H%M%S", time.localtime())
    cur_run_data_dir = "results/data/" + time_now
    if not os.path.exists(cur_run_data_dir):
        os.makedirs(cur_run_data_dir)
    print("The directory to store the data of this run has been created:  " + "\033[93m" + "{}".format(cur_run_data_dir) + "\033[0m", flush=True)
    return cur_run_data_dir